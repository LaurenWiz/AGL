...
```{r}
demo<-beach_dat[,c("Event_Date","Beach",
                        "Middle Latitude",
                        "Middle Longitude",
                        "Weight",
                        "numVolunteers")]
...
pop_up_data<-demo %>% 
  group_by(Beach,Event_Date,numVolunteers,Weight,`Middle Longitude`,`Middle Latitude`) %>% 
...
leaflet(pop_up_data) %>% 
  addTiles() %>% 
  addMarkers(~`Middle Longitude`,~`Middle Latitude`,
             popup=~htmlTable(data.frame(Event_Date,meanVolunteers,meanWeight)))
  #I inserted an html table into the map for each pin ont he map.  The formatting needs a little tweakeing but the concept works!  
```
